...
for j=1:repetitions
    ...
    for dim_loop = 1:1:length(D_full)
        ...
        combinations_necessary = channels_v + channels_v_ECG + channels_v_EEG;
        outputs = 0;
        total_vectors = 0;
        ...
        for i = 1:1:channels_v
            iMch1(i) = combinations{i,1};
            projM1_pos(i,:) = combinations{i,2};
            projM1_neg(i,:) = combinations{i,3};
        end

        for i = 1:1:channels_v_ECG
            iMch3(i) = combinations{i+channels_v,1};
            projM3_pos(i,:) = combinations{i+channels_v,2};
            projM3_neg(i,:) = combinations{i+channels_v,3};
        end

        for i = 1:1:channels_v_EEG
            iMch5(i) = combinations{i+channels_v_ECG+channels_v,1};
            projM5_pos(i,:) = combinations{i+channels_v_ECG+channels_v,2};
            projM5_neg(i,:) = combinations{i+channels_v_ECG+channels_v,3};
        end
        % Arousal
