...
for j=1:repetitions
    ...
    for dim_loop = 1:1:length(D_full)
        ...
        total_vectors = max([channels_v_EEG,channels_v_ECG,channels_v])+3*2;
        ...
        for i = 1:1:channels_v
            iMch1(i) = randSetVectors(i+6); %6 reserved at beginning from projm vectors
            projM1_pos(i,:) = randSetVectors(1);
            projM1_neg(i,:) = randSetVectors(2);
        end

        for i = 1:1:channels_v_ECG
            iMch3(i) = randSetVectors(i+6);
            projM3_pos(i,:) = randSetVectors(3);
            projM3_neg(i,:) = randSetVectors(4);
        end

        for i = 1:1:channels_v_EEG
            iMch5(i) = randSetVectors(i+6);
