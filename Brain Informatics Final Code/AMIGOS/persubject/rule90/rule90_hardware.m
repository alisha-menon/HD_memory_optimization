...
select = 1;
if (select == 1)
    HD_functions_rule90_hardware;     % load HD functions
else 
...
exaccuracy_A = zeros(subjects,repetitions);
exaccuracy_V = zeros(subjects,repetitions);
...
[L1_1, L2_1, L_SAMPL_DATA_1, SAMPL_DATA_1, L_SAMPL_DATA_1_test, SAMPL_DATA_1_test] = genTrainTestData (TS_COMPLETE_1, L_TS_COMPLETE_1, learningFrac, 'inorder', N, subject);
[L1_2, L2_2, L_SAMPL_DATA_2, SAMPL_DATA_2, L_SAMPL_DATA_2_test, SAMPL_DATA_2_test] = genTrainTestData (TS_COMPLETE_2, L_TS_COMPLETE_2, learningFrac, 'inorder', N, subject);
...
for j=1:repetitions
    ...
    for dim_loop = 1:1:length(D_full)
        ...
        total_vectors = channels_v+channels_v_ECG+channels_v_EEG;

        % Generating projm_pos
        seed_vector = genRandomHV(D);
        %seed_vector = csvread('seed_hv.csv');
        projm_pos = seed_vector;
        
        % Generating projm_neg
        seed_vector = foldedrule90(D, seed_vector, 1);
        projm_neg = seed_vector;
        ...
        for i = 1:total_vectors
            ...
            if (use_folding == 0)
                ...
                seed_vector = foldedrule90(D,seed_vector,num_folds);
                randSetVectors(i) = seed_vector;
            end
        end
        ...
        for i = 1:1:channels_v
            ...
            projM1_pos(i,:) = projm_pos;
            projM1_neg(i,:) = projm_neg;
        end

        for i = 1:1:channels_v_ECG
            iMch3(i) = randSetVectors(i+channels_v);
            projM3_pos(i,:) = projm_pos;
            projM3_neg(i,:) = projm_neg;
        end

        for i = 1:1:channels_v_EEG
            iMch5(i) = randSetVectors(i+channels_v+channels_v_ECG);
            projM5_pos(i,:) = projm_pos;
            projM5_neg(i,:) = projm_neg;
        end
        ...
        exaccuracy_A(subject,j) = acc_ex2;
        acc2
        acc_ex2
        ...
        exaccuracy_V(subject,j) = acc_ex1;
        acc1
        acc_ex1
...
exaaaccuracy_A = mean(exaccuracy_A);
exaaaccuracy_V = mean(exaccuracy_V);
