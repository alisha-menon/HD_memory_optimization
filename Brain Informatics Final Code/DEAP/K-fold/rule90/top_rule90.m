...
select = 1;
if (select == 1)
    HD_functions_rule90;     % load HD functions
else 
...
aaaaccuracy_sprdsht = zeros(3,2*length(D_full));
aaaaccuracy_sprdsht_ex_all = zeros(3,2*length(D_full));
kfold_accuracy_a = zeros(length(D_full),subjects);
kfold_accuracy_v = zeros(length(D_full),subjects);

max_average_accuracy_so_far = 0;


for learn_count = 1:1:length(D_full)
    D = D_full(learn_count); 
    D
    stored_vecs = cell(randCounter, 16);
    
    while (randCounter>0)
        rng('shuffle');

        [iMch1, iMch3, iMch5, iMch7, iMch9, chAM8, projM1_pos, projM1_neg, projM3_pos, projM3_neg, projM5_pos, projM5_neg, projM7_pos, projM7_neg, projM9_pos, projM9_neg] = rule_90(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, q, initItemMemories, genRandomHV, projRandomHV, randCounter);
        stored_vecs(randCounter,1) = {iMch1};
        stored_vecs(randCounter,2) = {iMch3};
        stored_vecs(randCounter,3) = {iMch5};
        stored_vecs(randCounter,4) = {iMch7};
        stored_vecs(randCounter,5) = {iMch9};
        stored_vecs(randCounter,6) = {chAM8};
        stored_vecs(randCounter,7) = {projM1_pos};
        stored_vecs(randCounter,8) = {projM1_neg};
        stored_vecs(randCounter,9) = {projM3_pos};
        stored_vecs(randCounter,10) = {projM3_neg};
        stored_vecs(randCounter,11) = {projM5_pos};
        stored_vecs(randCounter,12) = {projM5_neg};
        stored_vecs(randCounter,13) = {projM7_pos};
        stored_vecs(randCounter,14) = {projM7_neg};
        stored_vecs(randCounter,15) = {projM9_pos};
        stored_vecs(randCounter,16) = {projM9_neg};
    ...
    aaaaccuracy_sprdsht(1,learn_count*2-1) = acc_av_av_A;
    aaaaccuracy_sprdsht(1,learn_count*2) = acc_av_av_V;
    aaaaccuracy_sprdsht(2,learn_count*2-1) = acc_max_A;
    aaaaccuracy_sprdsht(2,learn_count*2) = acc_max_V;
    aaaaccuracy_sprdsht(3,learn_count*2-1) = acc_av_max_A;
    aaaaccuracy_sprdsht(3,learn_count*2) = acc_av_max_V;
    ...
    aaaaccuracy_sprdsht_ex_all(1,learn_count*2-1) = aacc_av_av_A_ex_all;
    aaaaccuracy_sprdsht_ex_all(1,learn_count*2) = aacc_av_av_V_ex_all;
    aaaaccuracy_sprdsht_ex_all(2,learn_count*2-1) = aaacc_max_A_ex_all;
    aaaaccuracy_sprdsht_ex_all(2,learn_count*2) = aaacc_max_V_ex_all;
    aaaaccuracy_sprdsht_ex_all(3,learn_count*2-1) = aaacc_av_max_A_ex_all;
    aaaaccuracy_sprdsht_ex_all(3,learn_count*2) = aaacc_av_max_V_ex_all;

    %%also put into spreadsheet
    if (max_average_accuracy_so_far < max(mean_AV_ex_all))
        %%also put into spreadsheet
        aaaaccuracy_matrix_data = acc_matrix;
        aaaaccuracy_matrix_data_ex_all = acc_matrix_ex_all;
        max_average_accuracy_so_far = max(mean_AV_ex_all);
    end
    
    iMch1 = stored_vecs(max_index, 1);
    iMch1 = iMch1{1};
    iMch3 = stored_vecs(max_index, 2);
    iMch3 = iMch3{1};
    iMch5 = stored_vecs(max_index, 3);
    iMch5 = iMch5{1};
    iMch7 = stored_vecs(max_index, 4);
    iMch7 = iMch7{1};
    iMch9 = stored_vecs(max_index, 5);
    iMch9 = iMch9{1};
    chAM8 = stored_vecs(max_index, 6);
    chAM8 = chAM8{1};
    projM1_pos = stored_vecs(max_index, 7);
    projM1_pos = projM1_pos{1};
    projM1_neg = stored_vecs(max_index, 8);
    projM1_neg = projM1_neg{1};
    projM3_pos = stored_vecs(max_index, 9);
    projM3_pos = projM3_pos{1};
    projM3_neg = stored_vecs(max_index, 10);
    projM3_neg = projM3_neg{1};
    projM5_pos = stored_vecs(max_index, 11);
    projM5_pos = projM5_pos{1};
    projM5_neg = stored_vecs(max_index, 12);
    projM5_neg = projM5_neg{1};
    projM7_pos = stored_vecs(max_index, 13);
    projM7_pos = projM7_pos{1};
    projM7_neg = stored_vecs(max_index, 14);
    projM7_neg = projM7_neg{1};
    projM9_pos = stored_vecs(max_index, 15);
    projM9_pos = projM9_pos{1};
    projM9_neg = stored_vecs(max_index, 16);
    projM9_neg = projM9_neg{1};
    
    for subject = 1:1:subjects
        subject
        fold_acc_a = zeros(1,5);
        fold_acc_v = zeros(1,5);
            features=inputs((subject-1)*40+1:subject*40,:);
            f_label_a_binary=features(:,239);
            f_label_v_binary=features(:,240);
    ...
    %         end
            ...
            % the selected data for training. The label dataset is in order from 1-7
            % and the data is also stacked one by one so that it is in order from 1-7
        rand_shuffle_1 = randperm(length(TS_COMPLETE_01));
        TS_COMPLETE_01 = TS_COMPLETE_01(rand_shuffle_1,:);
        L_TS_COMPLETE_01 = L_TS_COMPLETE_01(rand_shuffle_1);
        TS_COMPLETE_11 = TS_COMPLETE_11(rand_shuffle_1,:);
        TS_COMPLETE_21 = TS_COMPLETE_21(rand_shuffle_1,:);
        TS_COMPLETE_31 = TS_COMPLETE_31(rand_shuffle_1,:);
        TS_COMPLETE_41 = TS_COMPLETE_41(rand_shuffle_1,:);
        
        rand_shuffle_2 = randperm(length(TS_COMPLETE_02));
        TS_COMPLETE_02 = TS_COMPLETE_02(rand_shuffle_2,:);
        L_TS_COMPLETE_02 = L_TS_COMPLETE_02(rand_shuffle_2);
        TS_COMPLETE_12 = TS_COMPLETE_12(rand_shuffle_2,:);
        TS_COMPLETE_22 = TS_COMPLETE_22(rand_shuffle_2,:);
...
%% item memory and feature projection generation functions

function [iMch1, iMch3, iMch5, iMch7, iMch9, chAM8, projM1_pos, projM1_neg, projM3_pos, projM3_neg, projM5_pos, projM5_neg, projM7_pos, projM7_neg, projM9_pos, projM9_neg] = featproj_repeat(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, q, initItemMemories, genRandomHV, projRandomHV, randCounter)
    ...
    projM3_pos=projRandomHV(D,channels_v_EEG);
    projM3_neg=projRandomHV(D,channels_v_EEG);
    projM5_pos=projRandomHV(D,channels_v_GSR);
    projM5_neg=projRandomHV(D,channels_v_GSR);
    projM7_pos=projRandomHV(D,channels_v_BVP);
    projM7_neg=projRandomHV(D,channels_v_BVP);
    projM9_pos=projRandomHV(D,channels_v_RES);
    projM9_neg=projRandomHV(D,channels_v_RES);
...
function [iMch1, iMch3, iMch5, iMch7, iMch9, chAM8, projM1_pos, projM1_neg, projM3_pos, projM3_neg, projM5_pos, projM5_neg, projM7_pos, projM7_neg, projM9_pos, projM9_neg] = rule_90(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, q, initItemMemories, genRandomHV, projBRandomHV, randCounter)
    ...
    total_vectors = max([channels_v_EEG, channels_v_GSR, channels_v, channels_v_BVP, channels_v_RES])+2*5;
    ...
    for i = 1:1:channels_v_RES
        iMch9(i) = randSetVectors(i+10); %10 reserved at beginning from projm vectors
        projM9_pos(i,:) = randSetVectors(1);
        projM9_neg(i,:) = randSetVectors(2);
    end

    for i = 1:1:channels_v_BVP
        iMch7(i) = randSetVectors(i+10);
        projM7_pos(i,:) = randSetVectors(3);
        projM7_neg(i,:) = randSetVectors(4);
    end

    for i = 1:1:channels_v_GSR
        iMch5(i) = randSetVectors(i+10);
        projM5_pos(i,:) = randSetVectors(5);
        projM5_neg(i,:) = randSetVectors(6);
    end

    for i=1:1:channels_v_EEG
        iMch3(i) = randSetVectors(i+10);
        projM3_pos(i,:) = randSetVectors(7);
        projM3_neg(i,:) = randSetVectors(8);
    end

    for i=1:1:channels_v
        iMch1(i) = randSetVectors(i+10);
        projM1_pos(i,:) = randSetVectors(9);
        projM1_neg(i,:) = randSetVectors(10);
    end
end

function [iMch1, iMch3, iMch5, iMch7, iMch9, chAM8, projM1_pos, projM1_neg, projM3_pos, projM3_neg, projM5_pos, projM5_neg, projM7_pos, projM7_neg, projM9_pos, projM9_neg] = combvec(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, q, initItemMemories, genRandomHV, projBRandomHV, randCounter)
    [chAM8, iMch8] = initItemMemories (D, maxL, channels_v_EXG);
    ...
    combinations_necessary = channels_v_EEG+channels_v_GSR+channels_v+channels_v_BVP+channels_v_RES;
    outputs = 0;
    total_vectors = 0;
    ...
    for i = 1:1:channels_v_RES
        iMch9(i) = combinations{i,1};
        projM9_pos(i,:) = combinations{i,2};
        projM9_neg(i,:) = combinations{i,3};
    end

    for i = 1:1:channels_v_BVP
        iMch7(i) = combinations{i+channels_v_RES,1};
        projM7_pos(i,:) = combinations{i+channels_v_RES,2};
        projM7_neg(i,:) = combinations{i+channels_v_RES,3};
    end

    for i = 1:1:channels_v_GSR
        iMch5(i) = combinations{i+channels_v_RES+channels_v_BVP,1};
        projM5_pos(i,:) = combinations{i+channels_v_RES+channels_v_BVP,2};
        projM5_neg(i,:) = combinations{i+channels_v_RES+channels_v_BVP,3};
    end

    for i=1:1:channels_v_EEG
        iMch3(i) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR,1};
        projM3_pos(i,:) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR,2};
        projM3_neg(i,:) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR,3};
    end

    for i=1:1:channels_v
        iMch1(i) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR+channels_v_EEG,1};
        projM1_pos(i,:) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR+channels_v_EEG,2};
        projM1_neg(i,:) = combinations{i+channels_v_RES+channels_v_BVP+channels_v_GSR+channels_v_EEG,3};
    end
end