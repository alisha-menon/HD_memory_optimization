...
select = 1;
if (select == 1)
    HD_functions_cim_unoptimized;     % load HD functions
else 
...
while (randCounter>0)
    ...
    [iMch1, iMch3, iMch5, iMch7, iMch9, iMch8, chAM1, chAM3, chAM5, chAM7, chAM9, chAM8] = rule_90(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, initItemMemories, genRandomHV);
...
function [iMch1, iMch3, iMch5, iMch7, iMch9, iMch8, chAM1, chAM3, chAM5, chAM7, chAM9, chAM8] = rule_90(channels_v, channels_v_EEG, channels_v_GSR, channels_v_BVP, channels_v_RES, channels_v_EXG, D, maxL, initItemMemories, genRandomHV)
...
%     projM9_neg = zeros(channels_v_RES, D);
    ...
    for i = 1:1:channels_v_RES
        ...
        iMch9(i) = randSetVectors(randVs(i));
       
    end

    for i = 1:1:channels_v_BVP
        randVs = randperm(total_vectors);
        iMch7(i) = randSetVectors(randVs(i));
       
    end

    for i = 1:1:channels_v_GSR
        randVs = randperm(total_vectors);
        iMch5(i) = randSetVectors(randVs(i));
       
    end
    
    for i = 1:1:channels_v_EEG
        randVs = randperm(total_vectors);
        iMch3(i) = randSetVectors(randVs(i));
    ...
    for i = 1:1:channels_v
        randVs = randperm(total_vectors);
        iMch1(i) = randSetVectors(randVs(i));
